...
public class store {
    ...
    ListLinkInt<MenuClass> curMenuForUpdate = new ListLink<>();
    ...
    public String setMenuForUpdate(String theID)
    {
        curMenuForUpdate.clear();
        for (int i = 1; i < menu.getSize(); i++) {
            if(menu.get(i).getAffID().equals(curAff.getAid())){
                
                if (menu.get(i).getFoodid().equals(theID)) {
                    curMenuForUpdate.add(menu.get(i));
                }
    ...
    public ListLinkInt<MenuClass> getCurMenuFOrUpdate() {
        return  curMenuForUpdate;
    }

    public void setCurMenuFOrUpdate(ListLinkInt<MenuClass>  curMenuForUpdate) {
    ...
    public void UpdateMenu(String foodID){
        
        int index2 = findCurMenu(foodID);
        curMenu.remove(index2);
        index2 = findMenu(foodID);
        menu.remove(index2);
    }
}
